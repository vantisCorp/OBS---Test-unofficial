...
static inline int mem_pool_init(size_t default_pool_size)
{
	...
	mgr->default_pool_size = default_pool_size > 0 ? default_pool_size : MEM_POOL_DEFAULT_SIZE;
	mgr->pool_count = 0;
 ...
 */
static inline void mem_pool_stats(struct mem_pool *pool, size_t *total, size_t *used, size_t *peak)
{
	if (!pool)
		return;
...
static inline void *mem_pool_global_alloc(size_t size)
{
	...
	struct mem_pool *new_pool = mem_pool_create(size > mgr->default_pool_size ? size : mgr->default_pool_size);

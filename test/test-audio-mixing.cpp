...
TEST_F(AudioMixingTest, BasicMix) {
    ...
    // Verify result
    for (size_t i = 0; i < TEST_FRAMES; i++) {
        EXPECT_NEAR(expected[i], dest[i], 1e-6f) 
            << "Mismatch at frame " << i;
    }
}
...
TEST_F(AudioMixingTest, MixWithUnitVolume) {
    ...
    // Verify result
    for (size_t i = 0; i < TEST_FRAMES; i++) {
        EXPECT_NEAR(expected[i], dest2[i], 1e-6f) 
            << "Mismatch at frame " << i;
    }
}
...
TEST_F(AudioMixingTest, MixOddFrames) {
    ...
    for (size_t i = 0; i < odd_frames; i++) {
        EXPECT_NEAR(expected[i], small_dest[i], 1e-6f) 
            << "Mismatch at frame " << i;
    }
}
...
TEST_F(AudioMixingTest, LargeBuffer) {
    ...
    for (size_t i = 0; i < large_frames; i++) {
        EXPECT_NEAR(0.75f, large_dest[i], 1e-6f) 
            << "Mismatch at frame " << i;
    }
}
...
TEST_F(AudioMixingTest, NegativeValues) {
    ...
    for (size_t i = 0; i < frames; i++) {
        EXPECT_NEAR(expected[i], neg_dest[i], 1e-6f) 
            << "Mismatch at frame " << i;
    }
}
